...
#Total content
total_cont.all = read.table("/Users/danfab/comp_all/Total_TE_content.txt",header=T)
...
mean_ins_tab = mean_ins_tab[,-c(108:111)]
ncol(mean_ins_tab) #108
mean_ins_tab = mean_ins_tab[,c(108,1:107)]
...
################
#APPROACH 2 - NORMALIZED BY TOTAL COUNTS AND ASIN SQRT TRANSFORMED
################
...
write.table(normintmodel_output2,paste(output_path,"TotContNorm_interaction_model_ASINSQRT_allStat.txt",sep=""),quote = F,row.names = F,sep="\t")

#At Bonferroni alpha 0.05: 
round(table(normintmodel_output[normintmodel_output$regime_Bonf == "TRUE",]$DiffSelCont>0) / te.number,2) #18% pos, 2&

#At FDR 0.05
table(normintmodel_output$regime_fdr < 0.05) #41
table(normintmodel_output$study_fdr < 0.05) #101
table(normintmodel_output$inter_fdr < 0.05) #65

round(table(normintmodel_output[normintmodel_output$regime_fdr < 0.05,]$DiffSelCont>0) / te.number,2) #33% pos, 7&
table(normintmodel_output[normintmodel_output$regime_fdr < 0.05,]$DiffSelCont>0) #34 TEs sign more in selected, 7 sign less
nrow(normintmodel_output[normintmodel_output$regime_fdr < 0.05 & normintmodel_output$inter_fdr > 0.05,]) #5 with regime sign, but not interaction (4 negative, 11 positive)
nrow(normintmodel_output[normintmodel_output$regime_fdr < 0.05 & normintmodel_output$inter_fdr < 0.05,]) #36 with both significant
nrow(normintmodel_output[normintmodel_output$inter_fdr < 0.05,]) #65 with significant interaction
nrow(normintmodel_output[normintmodel_output$regime_fdr > 0.05 & normintmodel_output$inter_fdr < 0.05,]) #29 with only interaction
nrow(normintmodel_output[normintmodel_output$regime_fdr > 0.05 & normintmodel_output$inter_fdr > 0.05,]) #33 n.s. for both
