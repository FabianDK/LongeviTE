...
#Full model
table(stat_output.filter[stat_output.filter$PvalFull_Breed < BonfCut,]$Mean_Postponed > stat_output.filter[stat_output.filter$PvalFull_Breed < BonfCut,]$Mean_Early) #67 more in selected, 27 less
nrow(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]) #109 for diet
table(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]$Diff_HighCont > 0) #69 more in high, 40 less compared to control
table(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]$Diff_HighLow > 0) #94 more in high, 15 less compared to low
table(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]$Diff_ContLow > 0) #64 more in control, 45 less compared to low
...
table(stat_output.filter[stat_output.filter$Pval_Low < BonfCut,]$Diff_PostEarlyLow > 0) #In Low: Post (102) more than early (6)
table(stat_output.filter[stat_output.filter$Pval_Cont < BonfCut,]$Diff_PostEarlyCont > 0) #In control: Post (69) more than early (30)
table(stat_output.filter[stat_output.filter$Pval_High < BonfCut,]$Diff_PostEarlyHigh > 0) #In High: Post (7) LESS than early (91)
...
stat_output.filter.Edit = melt(data = stat_output.filter[,c(89,9:32)],value="flybase_name")
...
ggsave(filename = "all_sign_appr1_hoedjes.pdf",plot = boxplot.mean,width = 25,height = 25) #on dryad
...
nrow(hoedjes.stat.filt.sign) #94
nrow(hoedjes.stat.filt.signUP) #67
nrow(hoedjes.stat.filt.signDOWN) #27
