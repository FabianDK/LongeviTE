#Analysis of total genomic TE content + some plots (TE abundance calculated considering only the last 200 bp at the 3'-ends)
...
#Total TE content
total_cont.remo = read.table("/Users/danfab/Remolina/TE_maps/res_files/corrected/edit/output_stat/remolina_TE_total_content_3PRIME.txt",header=T)
total_cont.carn = read.table("/Users/danfab/Carnes/TE_maps/res_files/corrected/edit/output_stat/carnes_TE_total_content_3PRIME.txt",header=T)
total_cont.fab = read.table("/Users/danfab/Fabian/TE_maps/res_files/corrected/edit/output_stat/fabian_TE_total_content_3PRIME.txt",header=T)
total_cont.hoed = read.table("/Users/danfab/Hoedjes/TE_maps/res_files/corrected/edit/output_stat/hoedjes_TE_total_content_3PRIME.txt",header=T)
...
write.table(x = total_cont.all.edit, file = paste(output_path,"Total_TE_content_3PRIME.txt",sep=""), quote=F, row.names = F, sep="\t")
...
#pdf(file = paste(output_path,"insertion_diff_log2_All_sign_new_3PRIME.pdf",sep=""),width = 17,height = 10)
...
intmodel_output #only TIR significant
p.adjust(intmodel_output$study_P,method="fdr") < 0.05 #all
p.adjust(intmodel_output$regime_P[1:4],method="fdr") < 0.05 #only TIR
p.adjust(intmodel_output$inter_P[1:4],method="fdr") < 0.05 #only nonLTR

#Tukey tests for significant LTR and TIR
total.model = aov(nonLTR ~ Study+Regime+Regime*Study, data = total_cont.all)
summary(total.model)
...
#Save stats table
write.table(intmodel_output,paste(output_path,"Total_Content_Summary_Stats_3PRIME.txt",sep=""),row.names = F,quote=F,sep="\t")
...
ggsave(p.all, file=paste(output_path,"insertion_Classes_new_3PRIME.pdf",sep=""), height=9, width=14)
