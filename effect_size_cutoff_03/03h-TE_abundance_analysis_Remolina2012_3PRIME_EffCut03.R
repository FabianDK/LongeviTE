...
nrow(stat_output.filter) #61

#Plot filter
#pdf(file = paste(output_path1,"remolina_coverage_filter.pdf", sep =""),width = 8,height = 6)
...
nrow(stat_output.filter.sign) #19 are significant after Bonferroni
...
for (i in 1:nrow(stat_output.filter.sign)) {
  ...
  # Defining plot names
  combi.plot.path=paste(output_path,flybase.name,"_remolina_3PRIME.pdf", sep="")
...
remo.stat.filt = stat_output.filter
...
nrow(remo.stat.filt) #61
nrow(remo.stat.filt.UP) #53
nrow(remo.stat.filt.DOWN) #8

nrow(remo.stat.filt.sign) #19
nrow(remo.stat.filt.signUP) #17
nrow(remo.stat.filt.signDOWN) #2
...
table(remo.stat.filt$ConsCS) #2
table(remo.stat.filt$ConsSC) #4

remo.consistent.TEs = remo.stat.filt[remo.stat.filt$ConsCS == "TRUE" | remo.stat.filt$ConsSC == "TRUE",]
write.table(remo.stat.filt, paste(output_path1,"remolina_TE_stat_covfilter_withConsistent_3PRIME_200bp.txt",sep=""),quote = F, row.names = F,sep="\t")
...
head(total_cont.tab)
write.table(total_cont.tab,paste(output_path1,"remolina_TE_total_content_3PRIME.txt",sep=""),quote = F, row.names = F,sep="\t")
