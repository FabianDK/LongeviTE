...
library(mltools)
library(pBrackets)
setwd("/Users/danfab/Science/post-doc/EBI/Projects/Lifespan_TE/PopTE2/")

dir.create("/Users/danfab/comp_all/PopTE2",recursive = T)
output_path = "/Users/danfab/comp_all/PopTE2/"

#Script does:
#1) Fitlering of PopoolationTE2 output
#2) Calculation of average frequencies for each TE family
#3) Calculation of sum of frequencies over all detected insertions for each TE family (reviewer suggestion)
...
robert.2015 = read.table("/Users/danfab/Science/post-doc/EBI/Projects/Lifespan_TE/Robert_Plos2015_AvFreq_edit.txt",header=T)
head(robert.2015)
...
carnes.stat.filt.sign = carnes.stat.filt[carnes.stat.filt$ConsSC == "TRUE" | carnes.stat.filt$ConsCS == "TRUE",]
carnes.SC = carnes.stat.filt[carnes.stat.filt$ConsSC == "TRUE",]
carnes.CS = carnes.stat.filt[carnes.stat.filt$ConsCS == "TRUE",]

fabian.stat.filt.sign = fabian.stat.filt[fabian.stat.filt$ConsSC == "TRUE" | fabian.stat.filt$ConsCS == "TRUE",]
fabian.SC = fabian.stat.filt.sign[fabian.stat.filt.sign$ConsSC == "TRUE",]
fabian.CS = fabian.stat.filt.sign[fabian.stat.filt.sign$ConsCS == "TRUE",]

remo.stat.filt.sign = remo.stat.filt[remo.stat.filt$ConsSC == "TRUE" | remo.stat.filt$ConsCS == "TRUE",]
remo.SC = remo.stat.filt.sign[remo.stat.filt.sign$ConsSC == "TRUE",]
remo.CS= remo.stat.filt.sign[remo.stat.filt.sign$ConsCS == "TRUE",]

hoed.stat.filt.sign = hoed.stat.filt[hoed.stat.filt$ConsSC_L == "TRUE" | hoed.stat.filt$ConsCS_L == "TRUE" |
                                       hoed.stat.filt$ConsSC_H == "TRUE" | hoed.stat.filt$ConsCS_H == "TRUE" |
                                       hoed.stat.filt$ConsSC_C == "TRUE" | hoed.stat.filt$ConsCS_C == "TRUE" ,]

hoed.stat.filt.UP = hoed.stat.filt[hoed.stat.filt$ConsSC_L == "TRUE" | hoed.stat.filt$ConsSC_C == "TRUE" | hoed.stat.filt$ConsSC_H == "TRUE",]
hoed.stat.filt.DOWN = hoed.stat.filt[hoed.stat.filt$ConsCS_L == "TRUE" | hoed.stat.filt$ConsCS_C == "TRUE" | hoed.stat.filt$ConsCS_H == "TRUE",]

exclude = intersect(hoed.stat.filt.UP$TEfam, hoed.stat.filt.DOWN$TEfam) #in both categories
length(exclude) #18

hoed.SC = hoed.stat.filt.UP[!hoed.stat.filt.UP$TEfam %in% exclude,]
hoed.CS = hoed.stat.filt.DOWN[!hoed.stat.filt.DOWN$TEfam %in% exclude,]

#Annotation tab
ensembl_casey = read.table("/Users/danfab/Science/post-doc/EBI/Projects/Lifespan_TE/table_key/embl_repbase_mapping_from_Bergman_edit2.txt",header = T)
...
table(carnes.te2$dFreq > 0) #632 true, #109 false
table(fabian.te2$dFreq > 0) #4756 true, #3892 false
table(remo.te2$dFreq > 0) #2256 true, #2317 false
table(hoed.te2$dFreq > 0) #7643 true, #5643 false
#In all studies, average TE frequency was higher in selected than controls across all TEs
...
nrow(fabian.te2.filt) #8549 (wrong trim: 8557)
...
luniq(droplevels(carnes.te2.filt$TEfam)) #96
...
#Filter out TEs not thought to occur in Dmel
...
remo.te2.filt.CS$Type = "C>S" #doesnt exist
...
########################################################
#TAKE SUM OF FREQUENCIES FOR EACHT TE - REVIEWER SUGGESTION
########################################################
...
#POSSIBLY SHOW CORRELATION BETWEEN COUNTS AND DEVIATE ABUDNANCE
cor.test(carnes.te2.stat.pop.combi$CountS5,carnes.te2.stat.pop.combi$Sel_O5)
...
#####################################################
carnes.te2.stat.pop.combi.ns = carnes.te2.stat.pop.combi[carnes.te2.stat.pop.combi$ConsSC == "FALSE" & carnes.te2.stat.pop.combi$ConsCS == "FALSE",]
carnes.te2.stat.pop.combi.ns$Type = "ns"
fabian.te2.stat.pop.combi.ns = fabian.te2.stat.pop.combi[fabian.te2.stat.pop.combi$ConsSC == "FALSE" & fabian.te2.stat.pop.combi$ConsCS == "FALSE",]
fabian.te2.stat.pop.combi.ns$Type = "ns"
remo.te2.stat.pop.combi.ns = remo.te2.stat.pop.combi[remo.te2.stat.pop.combi$ConsSC == "FALSE" & remo.te2.stat.pop.combi$ConsCS == "FALSE",]
remo.te2.stat.pop.combi.ns$Type = "ns"
hoed.te2.stat.pop.combi.ns = hoed.te2.stat.pop.combi[!hoed.te2.stat.pop.combi$TEfam %in% union(hoed.SC$TEfam,hoed.CS$TEfam),]
hoed.te2.stat.pop.combi.ns$Type = "ns"
 
carnes.te2.stat.pop.combi.sign = carnes.te2.stat.pop.combi[carnes.te2.stat.pop.combi$ConsSC == "TRUE" | carnes.te2.stat.pop.combi$ConsCS == "TRUE",]
fabian.te2.stat.pop.combi.sign = fabian.te2.stat.pop.combi[fabian.te2.stat.pop.combi$ConsSC == "TRUE" | fabian.te2.stat.pop.combi$ConsCS == "TRUE",]
remo.te2.stat.pop.combi.sign = remo.te2.stat.pop.combi[remo.te2.stat.pop.combi$ConsSC == "TRUE" | remo.te2.stat.pop.combi$ConsCS == "TRUE",]
hoed.te2.stat.pop.combi.signSC = hoed.te2.stat.pop.combi[hoed.te2.stat.pop.combi$TEfam %in% hoed.SC$TEfam,]
hoed.te2.stat.pop.combi.signCS = hoed.te2.stat.pop.combi[hoed.te2.stat.pop.combi$TEfam %in% hoed.CS$TEfam,]
hoed.te2.stat.pop.combi.signSC$Type = "S>C"
hoed.te2.stat.pop.combi.signCS$Type = "C>S"

carnes.te2.stat.pop.combi.sign$Type = mgsub(c("TRUE","FALSE"),c("S>C","C>S"),carnes.te2.stat.pop.combi.sign$ConsSC)
fabian.te2.stat.pop.combi.sign$Type = mgsub(c("TRUE","FALSE"),c("S>C","C>S"),fabian.te2.stat.pop.combi.sign$ConsSC)
remo.te2.stat.pop.combi.sign$Type = mgsub(c("TRUE","FALSE"),c("S>C","C>S"),remo.te2.stat.pop.combi.sign$ConsSC)
hoed.te2.stat.pop.combi.sign = rbind(hoed.te2.stat.pop.combi.signSC,hoed.te2.stat.pop.combi.signCS)
...
cor.test(carnes.te2.stat.pop.combi.rob$FreqAv, carnes.te2.stat.pop.combi.rob$av_freq,method="spearman") #r = 0.09535112, p = 0.4
...
#Plot of correlation with Kofler2015
#pdf(paste(output_path,"FigS5_correlation_with_Kofler2015.pdf",sep=""), width=12, height=20)
...
plot(rank(fabian.te2.stat.pop.combi.rob$FreqAv), rank(fabian.te2.stat.pop.combi.rob$av_freq), xlim= c(0,115), ylim = c(0,115),
...
plot(rank(remo.te2.stat.pop.combi.rob$FreqAv), rank(remo.te2.stat.pop.combi.rob$av_freq),xlim= c(0,115), ylim = c(0,115),
...
plot(rank(hoed.te2.stat.pop.combi.rob$FreqAv), rank(hoed.te2.stat.pop.combi.rob$av_freq),xlim= c(0,115), ylim = c(0,115),
...
pdf(paste(output_path,"Fig2_Carnes_AvFreq_BoxPlots_log2FC_ConsDif.pdf",sep=""), width=8, height=6)
...
t.test(top10.c$av_freq ~ top10.c$Type.x)$p.value #0.36
text(1.5,1.14,"ns",cex=1.4)
brackets(x1 = 1, y1 = 1.05,
         x2 = 2, y2 = 1.05, 
         h = 0.05, ticks=NA, curvature = 0.5, type = 4,
...
pdf(paste(output_path,"Fig2_Fabian_AvFreq_BoxPlots_log2FC_ConsDif.pdf",sep=""), width=8, height=6)
m = matrix(c(1,1,2,2),nrow = 1,ncol = 4,byrow = TRUE)
layout(mat = m)
par(mar=c(4,4,1,1),font.axis=2,font.lab=2,lwd=1.2,cex=1.4)
...
axis(1,seq(1,2,1),labels = c("C>S","S>C"),cex.axis=1.2)
title(xlab = "Top 10", line = 2.2,cex.lab=1.4)
t.test(top10.c$av_freq ~ top10.c$Type.x)$p.value #0.29
text(1.5,1.14,"*",cex=1.4)
brackets(x1 = 1, y1 = 1.05,
         x2 = 2, y2 = 1.05, 
         h = 0.05, ticks=NA, curvature = 0.5, type = 4,
         col = 1, lwd = 2, lty = 1, xpd = FALSE)
...
pdf(paste(output_path,"Fig2_Remolina_AvFreq_BoxPlots_log2FC_ConsDif.pdf",sep=""), width=8, height=6)
m = matrix(c(1,1,2,2),nrow = 1,ncol = 4,byrow = TRUE)
layout(mat = m)
...
boxplot(remo.te2.stat.pop.combi.sign.rob$av_freq ~ as.factor(remo.te2.stat.pop.combi.sign.rob$Type.x), outpch = NA,axes = F,
        ylim = c(0,1.12),
        whisklty = 1, staplelwd = NA,col="white")
points(jitter(numfact(mgsub(c("S>C"),c(1),remo.te2.stat.pop.combi.sign.rob$Type.x)),factor=0.5), 
...
axis(1,seq(1,1,1),labels = c("S>C"),cex.axis=1.2)
axis(2,seq(0,1,0.2),labels = T,cex.axis=1.2)
# axis(4,seq(0,1,0.2),labels = F, line = 0,mgp=c(3, .5, 0))
title(ylab = "Frequency",line = 2.6,cex.lab=1.4)
title(xlab = "All", line = 2.2,cex.lab=1.4)
...
boxplot(top10.c$av_freq ~ as.factor(top10.c$Type.x),
        ...
        ylim = c(0,1.12),
        whisklty = 1, staplelwd = NA,col="white")
points(jitter(numfact(mgsub(c("S>C"),c(1),top10.c$Type.x)),factor=0.5), 
       top10.c$av_freq, 
       pch = top10.c$plotpch, 
       bg= top10.c$plotcol,cex=1.4)
axis(1,seq(1,2,1),labels = c("S>C"),cex.axis=1.2)
title(xlab = "Top 10", line = 2.2,cex.lab=1.4)
...
#Hoedjes
hoed.te2.stat.pop.combi.sign.rob.SC = hoed.te2.stat.pop.combi.sign.rob[hoed.te2.stat.pop.combi.sign.rob$TEfam %in% hoed.SC$TEfam,]
hoed.te2.stat.pop.combi.sign.rob.CS = hoed.te2.stat.pop.combi.sign.rob[hoed.te2.stat.pop.combi.sign.rob$TEfam %in% hoed.CS$TEfam,]
hoed.te2.stat.pop.combi.sign.rob.SC$plotcol = "red"
hoed.te2.stat.pop.combi.sign.rob.CS$plotcol = "blue"
hoed.te2.stat.pop.combi.sign.rob = rbind(hoed.te2.stat.pop.combi.sign.rob.SC,hoed.te2.stat.pop.combi.sign.rob.CS)
...
pdf(paste(output_path,"Fig2_Hoedjes_AvFreq_BoxPlots_log2FC_ConsDif.pdf",sep=""), width=8, height=6)
m = matrix(c(1,1,2,2),nrow = 1,ncol = 4,byrow = TRUE)
layout(mat = m)
...
axis(1,seq(1,2,1),labels = c("C>S","S>C"),cex.axis=1.2)
title(xlab = "Top 10", line = 2.2,cex.lab=1.4)
t.test(top10.c$av_freq ~ top10.c$Type.x)$p.value #0.16
text(1.5,1.14,"ns",cex=1.4)
brackets(x1 = 1, y1 = 1.05,
         x2 = 2, y2 = 1.05, 
         h = 0.05, ticks=NA, curvature = 0.5, type = 4,
...
################################################################################
#DEFINE CANDIDATE INSERTIONS
################################################################################
#stats all insertions - USE THIS FOR MS
p.val.vect2 = NULL
for (i in 1:nrow(fabian.te2.filt)){
  ...
  p.val.vect1 = c(Df,F.val,p.val)
  p.val.vect2 = rbind(p.val.vect2,p.val.vect1)
}
p.val.vect2
fabian.te2.filt$Df = p.val.vect2[,1]
fabian.te2.filt$Fval = p.val.vect2[,2]
fabian.te2.filt$Pval = p.val.vect2[,3]
...
write.table(fabian.te2.filt.edit, paste(output_path,"Fabian2018_StatsPerInsertion.txt",sep=""),quote=F,sep="\t",row.names = F)
...
fabian.te2.filt.test[1,26] = 2e-16
fabian.te2.filt.test[1,25] = 2e-16

names(fabian.te2.filt.test)[c(2,3,25)] = c("CHR","BP","P")
...
fabian.manh = gg.manhattan(df = fabian.te2.filt.test, 
                           threshold=max(fabian.te2.filt.test$Pval),  #fabian.te2.filt.sign$Pval THIS NEEDS CHANGE
                           hlight=mysnps, 
                           hlight2=NA,
                           col=mypalette, 
...
ggsave(fabian.manh, file=paste(output_path,"Fig3_Manhattan_plot_Bonf005_correct.pdf",sep=""), height=6, width=10)

#HOEDJES
#stats all insertions - anova, same as t-test but with var.equal = T - USE THIS FOR MS
p.val.vect2 = NULL
...
names(p.val.vect2) = c('Df.diet', 'Fval.diet','Pval.diet',
                       ...
                       'Df.inter','Fval.inter','Pval.inter')
...
write.table(hoed.te2.filt.new1,paste(output_path,"Hoedjes2019_StatsPerInsertion.txt",sep=""),quote=F,sep="\t",row.names = F)

#manhattan REGIME, HOEDJES2019
#bonf.005 = 0.05 / nrow(hoed.te2.filt.new)
hoed.te2.filt.test = hoed.te2.filt.new
#hoed.te2.filt.test = hoed.te2.filt.test[order(hoed.te2.filt.test$fdr.regime,decreasing = F),]
...
ggsave(hoed.manh, file=paste(output_path,"Fig3_Manhattan_Hoedjes2019_Regime_Bonf005.pdf",sep=""), height=6, width=10)

# #manhattan DIET
# hoed.te2.filt.test = hoed.te2.filt.new
# hoed.te2.filt.test.diet = hoed.te2.filt.test[,c("Chrom","Pos","p.val.diet","fdr.diet","dFreq")]
# 
# names(hoed.te2.filt.test.diet) = c("CHR","BP","P","FDR","dFreq")
# hoed.te2.filt.test.diet$CHR
# hoed.te2.filt.test.diet$CHR = factor(hoed.te2.filt.test.diet$CHR, levels = c("X", "2L", "2R", "3L", "3R","4"))
# hoed.te2.filt.test.diet$SNP = paste(hoed.te2.filt.test.diet$CHR,hoed.te2.filt.test.diet$BP,hoed.te2.filt.test.diet$TEfam,sep = ".")
# # hoed.te2.filt.test$SNP = hoed.te2.filt.test$TEfam
# mysnps = NA
# 
# head(hoed.te2.filt.test.reg)
# 
# mypalette <- c("#E2709A", "#CB4577", "#BD215B", "#970F42", "#75002B")
# mypalette <- c("black", "grey60")
# 
...
# detach("package:topGO", unload=TRUE)
# detach("package:org.Dm.eg.db", unload=TRUE)
# hoed.manh.diet = gg.manhattan(df = hoed.te2.filt.test.diet, 
#                          threshold=max(hoed.te2.filt.sign.diet$p.val.regime), 
#                          hlight=mysnps, 
#                          col=mypalette, 
#                          ylims=c(0,17), title="Hoedjes2019")
# hoed.manh.diet
# 
# ggsave(hoed.manh.diet, file="/Users/danfab/Science/post-doc/EBI/Projects/Lifespan_TE/PopTE2/candidates/For_Manuscript_Hoedjes/Manhattan_Hoedjes_Diet_Bonferroni005.pdf", height=6, width=10)
# 
# #manhattan INTER
# hoed.te2.filt.test = hoed.te2.filt.new
# hoed.te2.filt.test.inter = hoed.te2.filt.test[,c("Chrom","Pos","p.val.inter","fdr.inter","dFreq")]
# 
# names(hoed.te2.filt.test.inter) = c("CHR","BP","P","FDR","dFreq")
# hoed.te2.filt.test.inter$CHR
# hoed.te2.filt.test.inter$CHR = factor(hoed.te2.filt.test.inter$CHR, levels = c("X", "2L", "2R", "3L", "3R","4"))
# hoed.te2.filt.test.inter$SNP = paste(hoed.te2.filt.test.inter$CHR,hoed.te2.filt.test.inter$BP,hoed.te2.filt.test.inter$TEfam,sep = ".")
# # hoed.te2.filt.test$SNP = hoed.te2.filt.test$TEfam
# mysnps = NA
# 
# head(hoed.te2.filt.test.inter)
# 
# mypalette <- c("#E2709A", "#CB4577", "#BD215B", "#970F42", "#75002B")
# mypalette <- c("black", "grey60")
# 
...
# detach("package:topGO", unload=TRUE)
# detach("package:org.Dm.eg.db", unload=TRUE)
# hoed.manh.inter = gg.manhattan(df = hoed.te2.filt.test.inter, 
#                               threshold=max(hoed.te2.filt.sign.inter$p.val.regime), 
#                               hlight=mysnps, 
#                               col=mypalette, 
#                               ylims=c(0,17), title="Hoedjes2019")
# hoed.manh.inter
# 
# ggsave(hoed.manh.inter, file="/Users/danfab/Science/post-doc/EBI/Projects/Lifespan_TE/PopTE2/candidates/For_Manuscript_Hoedjes/Manhattan_Hoedjes_Interaction_Bonferroni005.pdf", height=6, width=10)
# 
...
for (i in 1:nrow(remo.te2.filt)){
  ...
  stat.vect = c(Df,F.val,p.val)
  p.val.vect2 = rbind(p.val.vect2,stat.vect)
}
p.val.vect2

remo.te2.filt$Df= p.val.vect2[,1]
remo.te2.filt$Fval = p.val.vect2[,2]
remo.te2.filt$Pval = p.val.vect2[,3
                                 ]
table(p.adjust(p.val.vect2[,3],"fdr") < 0.05) #0 significant
table(p.val.vect2[,3] < 0.05/length(p.val.vect2[,3])) #0 significant
remo.te2.filt$Bonf005 = remo.te2.filt$Pval < 0.05/length(remo.te2.filt$Pval)

remo.te2.filt.new = remo.te2.filt[,-c(1,4,8,15:20)]
write.table(remo.te2.filt.new,paste(output_path,"Remolina2012_StatsPerInsertion.txt",sep=""),quote=F,sep="\t",row.names = F)
...
for (i in 1:nrow(carnes.te2.filt)){
  ...
  F.val = summary(aov(asin(sqrt(int.tab[,1])) ~ int.tab$Reg))[[1]][["F value"]][1]
  p.val = summary(aov(asin(sqrt(int.tab[,1])) ~ int.tab$Reg))[[1]][["Pr(>F)"]][1]
  stat.val = c(Df,F.val,p.val)
  p.val.vect2 = rbind(p.val.vect2,stat.val)
}
p.val.vect2

carnes.te2.filt$Df = p.val.vect2[,1]
carnes.te2.filt$Fval = p.val.vect2[,2]
carnes.te2.filt$Pval = p.val.vect2[,3]
...
write.table(carnes.te2.filt.new,paste(output_path,"Carnes2015_StatsPerInsertion.txt",sep=""),quote=F,sep="\t",row.names = F)
...
hoed.edit$Diet = Diet
head(hoed.edit)
# hoed.edit1 = merge(hoed.edit,hoed.grp,by="TEfam")
# nrow(hoed.edit1)
# #233016
# nrow(hoed.edit)
# #312432
# names(hoed.edit1)[c(6,9)] = c("TypePop","TypeAb")
...
names(stat_output) = c("TEfam","Mean_Early","Mean_Postponed","diff_Freq","log2diff_Freq","Df_Breeding","Fval_Breeding","pvalFull_Breeding",'Df_Diet', 'Fval_Diet','pvalFull_Diet','Df_inter', 'Fval_inter',"pvalFull_inter")
...
write.table(stat_output,paste(output_path,"TableS6_Hoedjes_Frequency_diff_per_family.txt",sep=""),quote = F, row.names=F, sep="\t")
...
stat_output = as.data.frame(stat_output)
names(stat_output) = c("TEfam","Mean_Cont","Mean_Sel","diff_Freq","log2diff_Freq","Df_Regime","Fvalue_Regime","pval_Regime","Df_nested","Fvalue_nested","pval_nested")
for (i in 2:ncol(stat_output)){
  stat_output[,i] = numfact(stat_output[,i])
}
...
write.table(stat_output,paste(output_path,"TableS6_Fabian_Frequency_diff_per_family.txt",sep=""),quote = F, row.names=F, sep="\t")
...
write.table(stat_output,paste(output_path,"TableS6_Carnes_Frequency_diff_per_family.txt",sep=""),quote = F, row.names=F, sep="\t")
...
write.table(stat_output,paste(output_path,"TableS6_Remolina_Frequency_diff_per_family.txt",sep=""),quote = F, row.names=F, sep="\t")
