...
tab = read.table("/Users/danfab/Fabian/TE_maps/res_files/corrected/edit/fabian_all_TE_edited.txt") 
...
#Plot filter
#pdf(file = paste(output_path1,"fabian_coverage_filter.pdf", sep =""),width = 8,height = 6)
...
nrow(stat_output.filter.sign) #25
...
for (i in 1:nrow(stat_output.filter.sign)) {
  ...
  # Defining plot names
  combi.plot.path=paste(output_path,flybase.name,"_fabian_3PRIME.pdf", sep="")
...
fabian.stat.filt = stat_output.filter
...
nrow(fabian.stat.filt) #66
nrow(fabian.stat.filt.UP) #45
nrow(fabian.stat.filt.DOWN) #21

nrow(fabian.stat.filt.sign) #25
nrow(fabian.stat.filt.signUP) #17
nrow(fabian.stat.filt.signDOWN) #8
...
table(fabian.stat.filt$ConsCS) #3
table(fabian.stat.filt$ConsSC) #6

fabian.consistent.TEs = fabian.stat.filt[fabian.stat.filt$ConsCS == "TRUE" | fabian.stat.filt$ConsSC == "TRUE",]
write.table(fabian.stat.filt, paste(output_path1,"fabian_TE_stat_covfilter_withConsistent_3PRIME_200bp.txt",sep=""),quote = F, row.names = F,sep="\t")
...
head(total_cont.tab)
write.table(total_cont.tab,paste(output_path1,"fabian_TE_total_content_3PRIME.txt",sep=""),quote = F, row.names = F,sep="\t")
