...
dir.create("/Users/danfab/Carnes/TE_maps/res_files/corrected/edit/sign_TE_plots/")
dir.create("/Users/danfab/Carnes/TE_maps/res_files/corrected/edit/output_stat/")

#Settings
output_path = "/Users/danfab/Carnes/TE_maps/res_files/corrected/edit/sign_TE_plots/" #for many TE plots
...
tapply(tab.edit$pos,INDEX = tab.edit$TEfam,max)
...
nrow(stat_output.filter) #66

#Plot filter
#pdf(file = paste(output_path1,"carnes_coverage_filter_minAvCov",minAvCov,"_propCovCut",propCovCut,".pdf", sep =""),width = 8,height = 6)
...
nrow(stat_output.filter.sign) #53
...
for (i in 1:nrow(stat_output.filter.sign)) {
  ...
  # Defining plot names
  combi.plot.path=paste(output_path,flybase.name,"_carnes_3PRIME.pdf", sep="")
...
carnes.stat.filt = stat_output.filter
...
nrow(carnes.stat.filt) #66
nrow(carnes.stat.filt.UP) #43
nrow(carnes.stat.filt.DOWN) #23

nrow(carnes.stat.filt.sign) #53
nrow(carnes.stat.filt.signUP) #35
nrow(carnes.stat.filt.signDOWN) #18
...
table(carnes.stat.filt$ConsCS) #4
table(carnes.stat.filt$ConsSC) #19

carnes.consistent.TEs = carnes.stat.filt[carnes.stat.filt$ConsCS == "TRUE" | carnes.stat.filt$ConsSC == "TRUE",]
write.table(carnes.stat.filt, paste(output_path1,"carnes_TE_stat_covfilter_withConsistent_3PRIME_200bp.txt",sep=""),quote = F, row.names = F,sep="\t")
...
head(total_cont.tab)
write.table(total_cont.tab,paste(output_path1,"carnes_TE_total_content_3PRIME.txt",sep=""),quote = F, row.names = F,sep="\t")
...
carnes.stat.filt$TEsubclass = droplevels(carnes.stat.filt$TEsubclass)
carnes.stat.filt.sign$TEsubclass = droplevels(carnes.stat.filt.sign$TEsubclass)
carnes.stat.filt.signUP$TEsubclass = droplevels(carnes.stat.filt.signUP$TEsubclass)
carnes.stat.filt.signDOWN$TEsubclass = droplevels(carnes.stat.filt.signDOWN$TEsubclass)
...
#TIR significant
# [1] "non-LTR    twosided: 6.93258225572398e-09 | greater: 6.93258225572399e-09 | less: 0.999999999849909"
# [1] "TIR    twosided: 9.85129168195021e-05 | greater: 0.999997035720928 | less: 7.2880378551956e-05"

#C>S have TIR underrepresented
table(carnes.stat.filt.signDOWN$TEsubclass)
table(carnes.stat.filt.sign$TEsubclass)
