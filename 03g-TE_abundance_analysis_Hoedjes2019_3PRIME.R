...
#ONLY considering the last 200 bp at the 3'-ends
...
dir.create("/Users/danfab/Hoedjes/TE_maps/res_files/corrected/edit/sign_TE_plots/")
dir.create("/Users/danfab/Hoedjes/TE_maps/res_files/corrected/edit/output_stat/")

#Settings
output_path = "/Users/danfab/Hoedjes/TE_maps/res_files/corrected/edit/sign_TE_plots/"
...
for (i in 1:length(te.fams)) {
  ...
  tab.subset = tab.subset[tab.subset$pos >= max(tab.subset$pos) - three_prime+1,] #add +1 to have 200 pos
...
nrow(stat_output.filter) #68

#Plot filter
#pdf(file = paste(output_path1,"hoedjes_coverage_filter_minAvCov",minAvCov,"_propCovCut",propCovCut,".pdf", sep =""),width = 8,height = 6)
...
stat_output.filter.sign = stat_output.filter[stat_output.filter$Bonf_Full == "TRUE",]
nrow(stat_output.filter.sign) #49

#Full model
table(stat_output.filter[stat_output.filter$PvalFull_Breed < BonfCut,]$Mean_Postponed > stat_output.filter[stat_output.filter$PvalFull_Breed < BonfCut,]$Mean_Early) #45 more in selected, 4 less
nrow(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]) #55 for diet
table(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]$Diff_HighCont > 0) #52 more in high, 3 less compared to control
table(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]$Diff_HighLow > 0) #48 more in high, 7 less compared to low
table(stat_output.filter[stat_output.filter$PvalFull_Diet < BonfCut,]$Diff_ContLow > 0) #15 more in control, 40 less compared to low
nrow(stat_output.filter[stat_output.filter$PvalFull_Inter< BonfCut,]) #62 significant for interaction

#Within-Diet models for early vs late reproduction (separated by Diet regimes)
table(stat_output.filter[stat_output.filter$Pval_Low < BonfCut,]$Diff_PostEarlyLow > 0) #In Low: Post (44) more than early (2)
table(stat_output.filter[stat_output.filter$Pval_Cont < BonfCut,]$Diff_PostEarlyCont > 0) #In control: Post (50) more than early (3)
table(stat_output.filter[stat_output.filter$Pval_High < BonfCut,]$Diff_PostEarlyHigh > 0) #In High: Post (4) LESS than early (36)
...
stat_output.noFilt.sign = stat_output.noFilt[stat_output.noFilt$PvalFull_Breeding < alphaBonf/nrow(stat_output.noFilt),]
nrow(stat_output.noFilt.sign) #66

#pdf(file = paste(output_path1,"insertion_diff_Hoedjes_Filter_vs_NoFilter.pdf", sep =""),width = 7,height = 9)
...
for (i in 1:nrow(stat_output.filter.sign)) {
  ...
  # Defining plot names
  combi.plot.path=paste(output_path,flybase.name,"_hoedjes_3PRIME.jpg", sep="")
...
nrow(hoedjes.stat.filt) #68
nrow(hoedjes.stat.filt.UP) #59
nrow(hoedjes.stat.filt.DOWN) #9

nrow(hoedjes.stat.filt.sign) #49
nrow(hoedjes.stat.filt.signUP) #45
nrow(hoedjes.stat.filt.signDOWN) #4
...
table(hoedjes.stat.filt$ConsCSav) #1
...
table(hoedjes.stat.filt$ConsCS_L) #1
table(hoedjes.stat.filt$ConsSC_L) #26
...
table(hoedjes.stat.filt$ConsSC_C) #7
...
table(hoedjes.stat.filt$ConsCS_H) #26
table(hoedjes.stat.filt$ConsSC_H) #2

write.table(hoedjes.stat.filt, paste(output_path1,"hoedjes_TE_stat_covfilter_withConsistent_3PRIME_200bp.txt",sep=""),quote = F, row.names = F,sep="\t")
...
head(total_cont.tab)
write.table(total_cont.tab,paste(output_path1,"hoedjes_TE_total_content_3PRIME.txt",sep=""),quote = F, row.names = F,sep="\t")
...
hoedjes.stat.filt.sign = hoedjes.stat.filt.noFB[hoedjes.stat.filt.noFB$Bonf_Full == TRUE,]
hoedjes.stat.filt.signUP = hoedjes.stat.filt.sign[hoedjes.stat.filt.sign$Diff_PostEarly>0,]
hoedjes.stat.filt.signDOWN = hoedjes.stat.filt.sign[hoedjes.stat.filt.sign$Diff_PostEarly<0,]
