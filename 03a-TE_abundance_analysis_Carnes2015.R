...
nrow(stat_output.filter.sign) #107 
...
stat_output.filter.Edit = melt(data = stat_output.filter[,c(27,3:12)],value="flybase_name")
...
ggsave(filename = "all_sign_appr1_carnes.pdf",plot = boxplot.mean,width = 16,height = 16) #on dryad
...
nrow(carnes.stat.filt.sign) #107
nrow(carnes.stat.filt.signUP) #86
